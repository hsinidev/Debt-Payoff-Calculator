...
import type { Debt, PayoffResult } from '../types.ts';
import { calculatePayoff } from '../utils/DebtMath.ts';

const { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } = (window as any).Recharts;

const DebtInputRow: React.FC<{ debt: Debt; onUpdate: (id: string, field: keyof Debt, value: string | number) => void; onRemove: (id: string) => void; }> = ({ debt, onUpdate, onRemove }) => {
    return (
